...
function printBoard(board){
    for(let i =0; i<board.length; i++){
        ...
        for(let j=0 ;j<row.length; j++){
            rowString += row[j]
            if(j !== row.length-1) rowString += " | "     //adding seperators only in between not at last
        }
...
function checkWin(board){
    const lines = [
        [[0,0],[0,1],[0,2]], //row 0 where it represent the lin that make you win 
        [[1,0],[1,1],[1,2]], //row 1
        [[2,0],[2,1],[2,2]], //row 2
        [[0,0],[1,0],[2,0]], //col 0
        [[0,1],[1,1],[2,1]], //col 1
